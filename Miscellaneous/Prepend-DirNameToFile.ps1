...
if (Test-Path $folder) {
    ...
    New-Item $($folder + "\PrependedFiles") -ItemType Directory -Force
    foreach ($dir in $dirs) {
        $files = $null
        ...
        foreach ($file in $files) {
            try {
                ...
                Write-Host "TO  : $($folder + "\PrependedFiles\" + $dir.Name + "_" + $file.Name)"
                Copy-Item $file.FullName -Destination $($folder + "\PrependedFiles\" + $dir.Name + "_" + $file.Name) -Force -ErrorAction Stop
